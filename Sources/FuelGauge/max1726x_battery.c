                   ...
                   union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_STATUS:
        //aqui hay que editar el driver, si el current now es positivo, charging, sino el contrario
        /*
...
*/         
        //primero revisamos si tenemos la batería cargada, es decir >97%
        ...
        //ahora leemos el registro de corriente.
        ret = regmap_read(regmap, MAX1726X_CURRENT_REG, &reg);
        if (ret < 0)
            return ret;

        //Si la corriente es positiva, charging, sino discharging.
        if (max1726x_raw_current_to_uamps(priv, reg) > 0) val -> intval= POWER_SUPPLY_STATUS_CHARGING;
...
static int max1726x_init(struct max1726x_priv *priv)
{
    ...
    unsigned int reg;
    u32 fgrev, hibcfg;
	printk("Fuel gauge battery driver Kelboy2.0 V1.0\n");
	printk("Battery capacity configured to %d maH\n",(pdata->designcap)/2); 
    printk("Chip en addr 0x%02x\n",pdata->address);
...
static struct max1726x_platform_data *max1726x_parse_dt(struct device *dev)
{
    ...
    //Añadimos un apartado para obtener el "reg" =<> con el addres del device
    pdata->address = 0x36;
